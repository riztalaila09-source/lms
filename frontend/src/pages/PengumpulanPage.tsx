...
export default function PengumpulanPage() {
  ...
  const loadSubs = useCallback(async (aid: string) => {
    ...
    setLoading(true)
    try {
      const res = await assignmentClient.listSubmissions({ assignmentId: aid })
  ...
  useEffect(() => { loadSubs(assignmentId) }, [assignmentId, loadSubs])
  ...
  return (
    <AppLayout title={<><Icon as={LuInbox} /> Pengumpulan Tugas</>} subtitle="Lihat pengumpulan siswa dan beri nilai">
      ...
      <Dialog.Root open={gradeOpen} onOpenChange={(e) => setGradeOpen(e.open)}>
        ...
        <Dialog.Positioner>
          <Dialog.Content>
            <Dialog.Header><Dialog.Title>Beri Nilai — {target?.studentName}</Dialog.Title></Dialog.Header>
            <Dialog.Body>
              <Stack gap="12px">
                ...
                <Box>
                  <Text fontSize="11px" fontWeight="600" color={COLORS.muted} mb="4px">JAWABAN SISWA</Text>
                  <Box bg={COLORS.bg} p="10px" borderRadius="7px" fontSize="13px" whiteSpace="pre-wrap" maxH="180px" overflowY="auto">
                    {target?.content || <Text color={COLORS.muted}>{target?.timeTakenSeconds ? '(kuis pilihan ganda — dinilai otomatis)' : '(tidak ada teks)'}</Text>}
                  </Box>
                </Box>
                {target?.fileUrl && (
                  <Box>
                    ...
                    <Stack gap="4px">
                      {decodeLinks(target.fileUrl).filter((l) => l.url).map((l, i) => (
